...
export const ArtifactView: React.FC<ArtifactViewProps> = ({
  ...
  // Handle pre/post artifact content, apply think tag processing
  ...
  // Use useEffect to handle file opening
  useEffect(() => {
    const matches = Array.from(
      content.matchAll(
        /<boltAction[^>]*type="file"[^>]*filePath="([^"]*)"[^>]*>/g
  ...
  // When message is complete, mark all tasks as completed
  useEffect(() => {
    if (isComplete) {
      setFileStates((prev) => {
  ...
  // Modify single file restoration handler
  ...
  return (
    <div className="space-y-3">
              {/* Display text content before boltArtifact */}
      {preArtifactContent && (
            ...
            components={{
              code({ node, className, children, ...props }) {
                ...
                const isInline = !match;
                    ...
                    // Ensure children is string type
              ...
              pre({ children }) {
                // Return child elements directly without additional wrapping
                return children;
              },
              p({ children }) {
              ...
              a({ children, href }) {
                return (
                  <a
                    href={href}
                    className="text-[#000080] dark:text-blue-400 hover:text-[#000066] dark:hover:text-blue-300 hover:underline"
                    target="_blank"
              ...
              blockquote({ children }) {
                ...
                return (
                  <blockquote className="relative py-2 pl-4 my-2 text-sm text-gray-600 border-l-4 border-blue-200 rounded dark:border-[#000080] dark:text-gray-400 bg-blue-50 dark:bg-[#000080]/10 group">
                    <div
                    ...
                    {/* Gradient mask */}
                    {isCollapsed && (
                      <div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-blue-50 dark:from-[rgba(0,0,128,0.1)] to-transparent" />
                    )}
                    {/* Collapse/expand button */}
                    <button
                      onClick={() => setIsCollapsed(!isCollapsed)}
                      className="absolute p-1 text-[#000080] rounded-full bottom-1 right-2 dark:text-blue-400 hover:text-[#000066] dark:hover:text-blue-300 hover:bg-blue-100 dark:hover:bg-[#000080]/20"
                    >
                      <svg
                        className="w-4 h-4"
                        viewBox="0 0 24 24"
              ...
              thead({ children }) {
                return (
                                      <thead className="bg-blue-50 dark:bg-[#000080]/20">
                    {children}
                  </thead>
                );
              },
              ...
              th({ children }) {
                return (
                                        <th className="px-4 py-2 text-sm font-medium text-left text-[#000080] dark:text-blue-300 border dark:border-gray-700">
                    {children}
                  </th>
                );
              },
      ...
      <div className="bg-white dark:bg-[#1a1a1c] rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700/50">
        ...
        >
          ...
          <div className="flex items-center gap-2">
            {/* Only show Restore button when all tasks are completed */}
        ...
        {isExpanded && npmCommands.length > 0 && (
          <div className="border-t border-gray-200 dark:border-gray-700/50">
            {npmCommands.map((cmd, index) => (
              ...
              {/* Display text content after boltArtifact end */}
      {postArtifactContent && (
        <div className="text-gray-900 dark:text-gray-100 leading-relaxed prose dark:prose-invert prose-sm max-w-none ">
          <ReactMarkdown
            components={{
